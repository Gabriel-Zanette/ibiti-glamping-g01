...
test('API exige admin para identidade, assinatura para portador, isola pessoas e reserva uma só vez', async () => {
  ...
  try {
    ...
    const registration=await call(`/admin/people/${p.id}/onchain-registration`,{wallet:wallet.address});
    assert.equal(registration.status,200);
    const decoded=new Interface(['function registerWallet(address,bytes32)']).decodeFunctionData('registerWallet',registration.body.data);
    assert.equal(decoded[0],wallet.address);assert.equal(decoded[1],db.personChainId(p.id));
    assert.equal((await call(`/admin/people/${p.id}/onchain-registration`,{wallet:wallet.address},token)).status,401);
