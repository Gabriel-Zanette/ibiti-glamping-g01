...
export const ABI = [
  ...
  'function treasury() view returns(address)', 'function personOf(address) view returns(bytes32)',
  'function recoveries(address) view returns(address destination,address proposer,uint64 executeAfter,bytes32 caseHash)',
  'function recoverySource(address) view returns(address)', 'function registerWallet(address,bytes32)',
...
export class ChainSync {
  ...
  private async run(): Promise<ChainState> {
    ...
    // Seletor ausente é aceito apenas para leitura das versões históricas. Falha RPC não desativa validação.
    try { state.treasury = address(await this.token.treasury({blockTag})); }
    catch (e: any) { if (!(e.code === 'CALL_EXCEPTION' && e.data === '0x')) throw e; }
    if (state.treasury) {
      const linked = this.ledger.db.prepare('SELECT address,person_id FROM wallets WHERE person_id IS NOT NULL').all() as {address:string;person_id:string}[];
      const people = new Map(linked.map(w=>[w.address,w.person_id]));
      for (const m of movements) if (m.kind === 'reissue' && people.has(m.from)) people.set(m.to,people.get(m.from)!);
      state.registeredWallets = []; state.recoveryWallets = [];
      for (const [wallet,person] of people) {
        const [personId,recovery,source] = await Promise.all([this.token.personOf(wallet,{blockTag}),this.token.recoveries(wallet,{blockTag}),this.token.recoverySource(wallet,{blockTag})]);
        if (personId === this.ledger.personChainId(person)) state.registeredWallets.push(wallet);
        if (recovery.executeAfter !== 0n || source !== ZERO) state.recoveryWallets.push(wallet);
      }
    }
  ...
  async assertCurrent(personId: string) {
    ...
    const related = new Set(this.ledger.wallets(personId).map(w=>w.address));
      ...
      if (e?.name === 'RecoveryRequested' || e?.name === 'Reissued') return related.has(String(e.args.oldWallet).toLowerCase()) || related.has(String(e.args.newWallet).toLowerCase());
      if (e?.name === 'WalletRegistered') return related.has(String(e.args.wallet).toLowerCase());
