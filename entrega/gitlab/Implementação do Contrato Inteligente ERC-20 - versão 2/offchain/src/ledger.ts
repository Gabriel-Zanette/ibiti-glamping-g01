...
export type ChainState = { number: number; hash: string; timestamp: number; validFrom: number; validUntil: number; paused: boolean; maxPerWallet: number; owner: string; treasury?: string; registeredWallets?: string[]; recoveryWallets?: string[] };
...
export class Ledger {
  ...
  /// Identificador derivado do UUID aleatório, independente do CPF e do HMAC de identidade civil.
  personChainId(personId: string) { this.person(personId); return id("IBITI:person:" + personId); }
  ...
  applyBlock(state: ChainState, movements: Movement[], refundThrough = state.timestamp * 1000, refundFence = Number.MAX_SAFE_INTEGER) {
    this.transaction(() => {
      ...
      state = {...state, treasury: state.treasury ?? previous?.treasury ?? movements.find(m=>m.kind==='emission')?.to ?? state.owner};
      if (previous && state.number <= previous.number) {
        ensure(state.number === previous.number && state.hash === previous.hash, 'CHAIN_REORG');
        this.db.prepare('UPDATE metadata SET value=? WHERE key=?').run(JSON.stringify(state), 'chain');
      ...
      for (const m of movements) {
        ...
        if (m.kind === 'emission') {
          ...
          if (from.address !== to.address) {
            ...
            if (m.kind === 'reissue') {
              ensure(!from.person_id || !to.person_id || from.person_id === to.person_id, 'RECOVERY_PERSON_MISMATCH');
              if (from.person_id && !to.person_id) this.db.prepare('UPDATE wallets SET person_id=? WHERE address=?').run(from.person_id, to.address);
  ...
  quota(personId: string) {
    ...
    const wallets = this.wallets(personId).filter(w => !w.revoked && w.address !== (s?.treasury ?? s?.owner)?.toLowerCase());
    ...
    const identityMatched = s?.registeredWallets === undefined || wallets.every(w=>w.balance === 0 || s.registeredWallets!.includes(w.address));
    const recoveryPending = wallets.some(w=>s?.recoveryWallets?.includes(w.address));
    const eligible = member && s !== null && s.timestamp >= s.validFrom && !s.paused && balance <= s.maxPerWallet && identityMatched && !recoveryPending;
