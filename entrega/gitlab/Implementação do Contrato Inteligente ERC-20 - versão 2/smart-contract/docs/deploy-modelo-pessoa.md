# Histórico de publicação — Publicações e versões

> Revisão de 22/09/2026: a Parte 2 utiliza código técnico v3 local, ainda não publicado. Os registros abaixo documentam exclusivamente as implantações de 11/09 e não comprovam as novas salvaguardas.
...
A versão 1 não é proxy: editar arquivos não a atualiza. A versão 2 é outra emissão de teste, com outro endereço; não soma uma emissão extra ao mesmo IBIToken nem migra automaticamente posições, royalties ou hospedagens. Esse endereço corresponde somente à entrega anterior. Não repetir o deploy ao retomar a tarefa.
