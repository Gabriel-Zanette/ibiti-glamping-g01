...
describe('Hospedagens off-chain com IBIToken real', function () {
  it('compra, solicita, cancela, transfere, reemite e recebe royalty sem markRedeemed', async function () {
    ...
    await (await token.registerWallet(alice.address,db.personChainId(p.id))).wait();
    await (await token.registerWallet(bob.address,db.personChainId(q.id))).wait();
    ...
    await sync.sync();assert.equal(db.quota(q.id).available,0);
    const request=await token.recoveries(bob.address);
  ...
  it('rejeita rede errada e consumo legado sem migração', async function () {
    ...
    const token = await chain.deploy('LegacyRedemptionFixture', [admin.address,150,1735689600,1861919999,ethers.ZeroAddress]);
    const receipt = await token.deploymentTransaction()!.wait();
    const db = new Ledger(':memory:', 'integration'.repeat(8), 'local');
    ...
    await(await token.registerWallet(a.address,ethers.id('a'))).wait();
...
it('cadastro on-chain e tesouraria permanecem coerentes no serviço após troca de owner', async () => {
  const [admin,a,b] = chain.signers;
  const token=await chain.deploy('IBIToken',[admin.address,150,1735689600,1861919999,ethers.ZeroAddress]);
  const receipt=await token.deploymentTransaction()!.wait();
  const db=new Ledger(':memory:','integration'.repeat(8),'identity-test');
  try {
    const p=db.registerPerson('52998224725');db.setVerified(p.id,true);db.linkWallet(p.id,a.address);
    const sync=new ChainSync(db,chain.provider,await token.getAddress(),receipt!.blockNumber,31337);
    await(await token.registerWallet(a.address,db.personChainId(p.id))).wait();
    await(await token.primaryPurchase(a.address,5,ethers.id('a'))).wait();await sync.sync();
    assert.equal(db.quota(p.id).available,5);
    await(await token.transferOwnership(a.address)).wait();await(await token.connect(a).acceptOwnership()).wait();await sync.sync();
    assert.equal(db.quota(p.id).available,5);assert.equal(db.state()!.treasury,admin.address.toLowerCase());
    db.linkWallet(p.id,b.address);
    await(await token.connect(a).registerWallet(b.address,ethers.id('cadastro-divergente'))).wait();
    await(await token.connect(a).primaryPurchase(b.address,1,ethers.id('b'))).wait();await sync.sync();
...
it('recuperação sem saldo revoga sessão e preserva cotas e royalties no cadastro original', async () => {
  ...
  const token=await chain.deploy('IBIToken',[admin.address,150,1735689600,1861919999,ethers.ZeroAddress]);
  const receipt=await token.deploymentTransaction()!.wait();
  const db=new Ledger(':memory:','integration'.repeat(8),'zero-recovery');
  try {
    const p=db.registerPerson('52998224725');db.setVerified(p.id,true);db.linkWallet(p.id,a.address);
    const q=db.registerPerson('11144477735');db.setVerified(q.id,true);db.linkWallet(q.id,b.address);
    for(const [w,person,n] of [[a,p,5],[b,q,1]] as const) {
