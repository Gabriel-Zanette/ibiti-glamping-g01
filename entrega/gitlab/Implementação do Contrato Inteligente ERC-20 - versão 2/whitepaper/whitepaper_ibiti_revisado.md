...
### Edição revisada em 22/09/2026 | IBIToken v3
...
O documento abrange a emissão e a circulação de IBT, a distribuição de royalties, o controle de experiências por pessoa, a governança e as responsabilidades operacionais. A implementação de referência é a versão técnica 3, preparada para a segunda entrega acadêmica e validada em EVM local. Esta revisão ainda não possui implantação pública. Os contratos v1 e v2 publicados anteriormente na Sepolia são históricos e não incorporam suas novas regras.
...
Cada IBT é uma unidade inteira de um token fungível ERC-20. Das 150 unidades, 50 ficam inicialmente reservadas à administração e 100 estão potencialmente disponíveis à colocação. O limite é de 20 IBT por pessoa, somadas todas as carteiras associadas ao mesmo identificador opaco no contrato. A identidade civil é verificada fora da cadeia; seu vínculo com cada carteira é registrado antes da compra e não pode ser reatribuído. A correção e unicidade desse cadastro continuam sob responsabilidade da operação.
...
A arquitetura prevista para operação comercial utiliza Ethereum Mainnet, IBT no padrão ERC-20 e BRZ para liquidação. A validação desta revisão utiliza EVM local e tBRL, moeda de teste sem lastro financeiro; as implantações históricas operam na Sepolia. A infraestrutura financeira prevista e seu estágio de integração são descritos no capítulo 10.
...
| Limite de participante | 20 IBT | Agregado das carteiras da mesma pessoa no contrato |
| Participação na receita | 15% do faturamento bruto | Não é rentabilidade anual de 15% |
| Distribuições | 8 períodos semestrais | Encerramentos civis mínimos impostos pelo código |
...
A jornada primária exige identificação, prova de controle da carteira e registro do identificador opaco no contrato antes da compra. O código exige esse vínculo e o teto agregado, mas não valida documentos civis. Para hospedagens, o backend exige cadastro aprovado, carteira vinculada e concordância com o identificador registrado on-chain.
...
A tBRL é independente do IBT: seu saldo serve aos testes de compra, depósito e saque. Criar unidades dessa moeda não altera a emissão de 150 IBT ou as cotas de experiência.
...
O contrato gera oito semestres civis em UTC a partir de validFrom, necessariamente 1º de janeiro, e exige validade até o último segundo do quarto ano. Cada período só pode ser registrado depois do encerramento, em sequência. Para 2027–2030, as primeiras datas possíveis são 01/07/2027, 01/01/2028, 01/07/2028, 01/01/2029, 01/07/2029, 01/01/2030, 01/07/2030 e 01/01/2031, às 00h00 UTC. O cálculo respeita anos bissextos.

Reportes atrasados são permitidos, inclusive após a expiração. O corte permanece no saldo da transação de apuração: atraso não recompõe a posição no encerramento contábil. A expiração impede circulação, mas não apaga obrigações, saques ou recuperação. Os direitos acumulados dependem também dos instrumentos da operação.
...
A pessoa recebe as condições, é identificada e comprova controle da carteira. O administrador registra o identificador opaco de sua pessoa no contrato. Com stablecoin configurada e preço unitário fixado uma vez, o comprador autoriza o valor e chama buyPrimary. A mesma transação transfere stablecoin à tesouraria e entrega IBT; falha de pagamento, cadastro, limite ou estoque reverte toda a compra.

Somente no modo sem stablecoin, primaryPurchase permite entrega administrativa após pagamento externo, cuja comprovação permanece fora da blockchain. Essa alternativa não executa PIX nem oferece atomicidade com o pagamento bancário. Ao configurar stablecoin, a entrega administrativa externa é desabilitada.
...
Saídas diretas da tesouraria por transfer ou transferFrom são bloqueadas para impedir entrega sem passar pela compra. O preço da demonstração é 37.055,19 tBRL por IBT, correspondendo ao valor de referência arredondado; a compra de 100 soma 3.705.519,00 tBRL. Esse arredondamento operacional difere da referência de valuation com precisão integral da planilha, sem alterá-la. tBRL não representa reais nem uma oferta comercial.
...
A governança é administrativa, sem votação dos titulares, DAO ou proxy de atualização. Recuperações exigem anúncio público com hash do processo, espera mínima de 48 horas e podem ser canceladas pela origem ou pelo administrador, inclusive durante pausa. Outros poderes administrativos não possuem espera obrigatória. O prazo é uma salvaguarda técnica desta implementação; não demonstra política contratual ratificada pela IBITI.

| Configuração | Alterável na mesma implantação? | Condição |
| --- | --- | --- |
| Oferta, casas decimais e limite pessoal | Não | Fixados na implantação e no código |
...
| Proprietário | Sim | Indicação e aceite; tesouraria não muda |
| Tesouraria | Não | Fixada na emissão, inicialmente igual ao administrador |
| Preço unitário da compra | Configuração inicial única | Menor unidade da stablecoin |
| Pausa | Sim | Sem extinguir todo direito acumulado |

## 8.2 Recuperação de participante

A recuperação deve ser anunciada com 48 horas de antecedência. Origem e destino ficam impedidos de movimentar IBT enquanto o pedido estiver pendente; pagamentos à origem também ficam suspensos. O destino precisa estar vazio, sem royalties pendentes e não pode ser tesouraria, administrador atual ou indicado, endereço com código de contrato ou carteira de outra pessoa. Uma carteira nova recebe o mesmo identificador da origem.

Depois da espera, reissue transfere o saldo por queima e emissão compensatórias, sem alterar a oferta, revoga a origem e redireciona royalties pendentes por período. Também funciona sem IBT, quando há valores a receber, e após a expiração. Pagamentos já realizados permanecem no histórico original. Mudança de administrador invalida anúncios do administrador anterior, exigindo cancelamento e novo anúncio, mesmo se o administrador anterior voltar ao cargo.
...
A troca de proprietário exige indicação e aceite. A tesouraria é um endereço separado e imutável, inicialmente igual ao administrador do deploy. Reserva, estoque, exceção ao teto e recebimento da compra permanecem vinculados à tesouraria. O novo administrador pode operar vendas externas quando habilitadas e reportar receitas sem receber o estoque. A pessoa que assume a administração não recebe isenção de teto em sua carteira pessoal.

A tesouraria imutável reduz o acoplamento entre governança e estoque, mas não pode ser substituída por essa implantação. Perder sua chave afeta seus recebíveis; comprometer a chave administrativa ainda permite abusar de registro de identidades, pausa e demais poderes. O anúncio de recuperação oferece prazo de contestação, mas não substitui controles de custódia e supervisão.
...
A recuperação implementada destina-se à recomposição de acesso da mesma pessoa. Sucessão para outra pessoa exige procedimento jurídico e técnico específico; não utiliza esse mecanismo para alterar silenciosamente o titular. Liberação de reserva, pausa, recuperação e mudança de autoridade são atos identificáveis nos registros da operação.
...
| Colocação | buyPrimary, primaryPurchase | Compra atômica em stablecoin ou entrega externa apenas sem stablecoin |
| Identificação | registerWallet, personOf, personBalance | Vínculo opaco permanente e agregado por pessoa |
| Limites e reserva | Regras de movimentação e reduceReserve | Teto pessoal e piso da tesouraria |
...
| Receita | reportRevenue, periodEnd | Exige semestre encerrado; calcula 15% e rateia saldo corrente |
...
| Recuperação | requestRecovery, cancelRecovery, reissue | Anúncio, contestação, espera e recuperação de saldo e recebíveis |
| Administração | Indicação e aceite de propriedade | Duas etapas; não altera tesouraria |
...
A transferência primária parte da tesouraria e respeita estoque, reserva, cadastro, limite pessoal e restrições. Entre participantes, o destinatário deve já possuir IBT e estar registrado, salvo retorno à tesouraria. A versão atual não permite venda secundária direta a qualquer carteira nova só porque ela foi aprovada fora da blockchain.

O vínculo opaco da pessoa verificada é registrado no contrato antes da compra. A função ERC-20 approve concede a outro endereço permissão de movimentação de saldo, independentemente da verificação de identidade.
...
Cada movimentação entre pessoas verifica se a soma das carteiras da pessoa destinatária ultrapassaria 20 IBT, inclusive transferFrom. Movimentos internos entre carteiras do mesmo identificador conservam o agregado. O teto limita a posição econômica sobre a qual serão calculados royalties. A associação fraudulenta de uma mesma pessoa a identificadores distintos ainda depende da integridade da verificação externa; a blockchain não descobre identidades civis duplicadas.
...
O ativo de pagamento deve ter comportamento compatível. A compra e o depósito verificam o aumento exato do saldo recebido e rejeitam recebimento inferior ao devido. Rebasing, bloqueios do emissor e particularidades de saque ainda podem afetar o modelo; stablecoins com taxa não são suportadas. Declarar ERC-20 não comprova compatibilidade com todos os pressupostos econômicos do rateio.
...
| Circulação | Teto pessoal e cadastro; isenção exclusiva da tesouraria |

A conservação de oferta, o rateio e o pagamento único são propriedades verificadas nos testes funcionais. A suficiência financeira depende de um ativo de pagamento compatível. Na troca de proprietário, a tesouraria e suas regras permanecem estáveis, pois a mudança de autoridade não redistribui saldos.

## 9.6 Compilação e verificação

A implementação utiliza Solidity 0.8.34, otimizador com 200 execuções, alvo EVM Osaka e OpenZeppelin 5.6.1. A revisão técnica v3 foi compilada com solc e seus fluxos foram executados em Anvil. A correspondência exata no Sourcify [11] pertence somente à implantação histórica v2, não a esta revisão ainda sem deploy público.

A validação funcional cobre oferta, reserva, limites, transferências, recuperação, apuração, financiamento, pagamento único e integração com o livro de cotas. A validação inclui testes Solidity, regressões temporais, scripts Remix executados em Anvil e o serviço off-chain. O relatório da entrega identifica a contagem e os comandos; os testes no navegador do Remix não foram reexecutados nesta revisão.
...
Após a prova de controle, a API prepara o registro do identificador opaco para assinatura administrativa no contrato, sem custodiar chave. O identificador é derivado do UUID aleatório da pessoa, separado do HMAC de CPF. Não há fluxo público de desvincular e reassociar uma carteira a outra pessoa. Recuperação, erro cadastral e sucessão exigem procedimento que preserve histórico.
...
Na versão técnica 3, o serviço consulta também a tesouraria, os identificadores por carteira e os anúncios de recuperação. Divergência entre cadastro e contrato ou recuperação pendente impede novas hospedagens.
...
A versão técnica 3 é a implementação local desta edição e ainda não possui endereço público. A versão 2 histórica está publicada na Sepolia, chain ID 11155111; seus registros constam do Apêndice C. Ambas têm 150 IBT, zero casas decimais e reserva inicial de 50, mas a versão 2 não impõe as novas salvaguardas de identidade, calendário e recuperação.
...
O contrato não utiliza proxy de atualização. Uma nova publicação cria outro endereço e outra emissão, sem transferir automaticamente saldos, cotas ou obrigações. A implantação histórica do Apêndice C não corresponde ao código revisado desta edição. Uma publicação da versão 3 deve registrar endereço, rede, recibo e fontes próprios e definir explicitamente eventual transição.
...
Este whitepaper descreve o modelo do IBIToken e sua implementação acadêmica revisada, validada em EVM local, distinguindo as versões históricas na Sepolia. A emissão de testes não é uma emissão comercial na Mainnet.
...
A implementação local desta edição é a versão técnica 3, sem publicação pública confirmada. Seus parâmetros econômicos são 150 IBT, reserva inicial 50, teto pessoal 20, validade de 01/01/2027 a 31/12/2030 e oito encerramentos semestrais. O registro abaixo é evidência histórica da versão 2, preservada para distinguir código entregue e código publicado.

## C.1 Implantação histórica da versão 2
...
## C.2 Vigência histórica e identificação da rede
...
Consulta pública: fonte histórica verificada no Sourcify [11]; transação e rede identificadas na tabela acima.

# Apêndice D. Referências

## D.1 Documentação técnica do projeto

A descrição funcional tem como base o código revisado do IBIToken v3, o serviço de cotas por pessoa e seus testes. O conjunto técnico do projeto reúne o código-fonte, as regras de negócio, os procedimentos de execução, o relatório de testes e o registro de implantação.
