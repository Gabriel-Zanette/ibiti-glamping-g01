...
export function updateSimulations(root = document) {
  for (const box of root.querySelectorAll("[data-simulation]")) {
    ...
    out.textContent = `${units} IBT · ${units} cota${units === 1 ? "" : "s"} · ${reference ? money(reference.unitCents * units) : "Consultando valor…"}`;
  }
}
